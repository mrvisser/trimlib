...
`namespace="t"` is specifying the **namespace** of the library; and `href="tld.html"` is specifying
a document that contains the actual Trimpath JST templates (note that for XSS security, the path to
tld.html must be on the same domain as the html document).

`jQuery('#content').trimlib({namespace: 't', template: 'hello', data: {world: 'World'}});`

This tells trimlib to render the template `hello` in the library with namespace `t` with data
`{world: 'World'}`, and insert it into the element with id `content`. Pretty straight-forward.
...
<link rel="trimlib" type="text/html" namespace="t" href="tld.html" />
...
<t:task-list title="My Task List" items="javascript:myTaskItems" />
...
<script type="text/javascript">
...
Using these custom tags would be pretty lame if you couldn't render the inner body.
That's why the `__body` variable exists. In your JST, if you access the `__body` variable, the body
of the custom tag will be recursively expanded and inserted into the location of the variable in the
...
<t:section title="My Things">
	...
	<t:task-list title="Tasks" items="javascript:myTaskItems" />
	<t:pictures pictures="javascript:myPictures" />
</t:section>
...
<script type="text/javascript">
...
the inner body of the tag will never be expanded.

For a slightly more complex demonstration of how TrimLib can be used, I developed it while writing a static
